function buildReqParams (params, parameterList, _definitions = {}) {
  ...
  const totalBodyParam = extractBodyParam(bodyParams, _definitions)
  const buildedBodyParams = buildParams(totalBodyParam).concat(buildedFormDataParams)
...
function buildParams (parameterList = []) {
  return parameterList.map(item => {
    ...
    let type = item.schema ? item.schema.type : item.type
    var param = {
      key: item.name,
...
export { buildReqParams, buildResponse, handleSwaggerSchema, buildParams }
